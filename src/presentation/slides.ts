...
export const slides: Slide[] = [
  ...
  },
  {
    ...
    goals: [
      ...
      {
        title: 'Сформировать кадровый резерв L1 → L2 → L3',
        icon: 'GraduationCap',
        text: 'Запустить «Школу поддержки»: 2 сотрудника L1 повышены до L2, 1 сотрудник L2 (Онищенко / Медведев) осваивает смежный стек — например, инфраструктуру — для взаимозаменяемости.',
        result: 'Команда закрывает задачи без привязки к конкретному человеку',
        effect: 'Исключение риска «эффекта одного эксперта», когда уход ключевого сотрудника парализует процесс',
      },
      {
        title: 'Создать AI-центр анализа корневых причин (RCA) и профилактики',
        icon: 'Microscope',
        text: 'Нейросеть анализирует все инциденты за год, выявляет глубинные причины — код, конфигурация, нагрузка — и предлагает архитектурные изменения.',
        result: 'Количество повторных инцидентов снижается на 80%',
        effect: 'Исключение «вечных болячек» — экономия времени разработчиков на тушение пожаров, эквивалент 5+ full-time инженеров',
      },
      {
        title: 'Внедрить гарантированный SLA с финансовой ответственностью',
        icon: 'FileCheck',
        text: 'Для каждого бизнес-процесса — запись, выписка, выплаты — установлены SLO с автоматическими компенсациями бизнесу за нарушение: при простое более 15 минут списывается 10% бюджета IT-департамента.',
        result: 'Прямая подотчётность IT перед бизнесом',
